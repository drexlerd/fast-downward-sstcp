...
CEGAR::CEGAR(
    ...
    : task_proxy(*task),
      ...
      abstract_search(task_properties::get_operator_costs(TaskProxy(*tasks::g_root_task))),
      timer(max_time),
      debug(debug) {
    assert(max_states >= 1);
...
bool CEGAR::is_unsolvable() {
    int init_id = abstraction->get_initial_state().get_id();
    return (abstract_search.get_h_value(init_id) == INF) ? true : false;
}
}
