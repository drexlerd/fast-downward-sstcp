...
CartesianSet& AbstractState::get_cartesian_set() {
    return cartesian_set;
}

void AbstractState::make_concrete_cartesian_set(
  extra_tasks::DomainAbstractedTask* dat,
  const std::vector<int> &concrete_domain_sizes) {
    CartesianSet concrete_cartesian_set = CartesianSet(concrete_domain_sizes);
    for (int var = 0; var < (int)concrete_domain_sizes.size(); ++var) {
        Bitset &abstract_values = cartesian_set.get_bitset_ref_from_var(var);
        Bitset &concrete_values = concrete_cartesian_set.get_bitset_ref_from_var(var);
