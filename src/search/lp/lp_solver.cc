...
LPConstraint::LPConstraint(const LPConstraint &other) {
    variables = vector<int>(other.variables);
    coefficients = vector<double>(other.coefficients);
    assert(&variables != &other.variables);
    assert(&coefficients != &other.coefficients);
    lower_bound = other.lower_bound;
    upper_bound = other.upper_bound;
}

LPConstraint& LPConstraint::operator=(const LPConstraint &other) {
...
bool LPSolver::has_optimal_solution() const {
    assert(is_solved);
    if (lp_solver->isProvenPrimalInfeasible()) {
        cout << "Solution proven infeasible" << endl;
    }
    if (lp_solver->isProvenDualInfeasible()) {
        cout << "Solution proven dual infeasible" << endl;
    }
    if (!lp_solver->isProvenOptimal()) {
        cout << "Solution proven not optimal" << endl;
    }
    try {
