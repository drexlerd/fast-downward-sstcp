...
void add_scp_options_to_parser(OptionParser &parser) {
    ...
    parser.add_option<shared_ptr<Saturator>>(
        "diversified_saturator",
        "extra state dependent saturator that is run after the other saturators on the remaining costs",
        OptionParser::NONE);
}
...
shared_ptr<Heuristic> get_max_cp_heuristic(
    ...
    parser.add_option<shared_ptr<OperatorMaskGenerator>>(
        "operator_mask_generator",
        "determine which operators are handled state-dependent",
        "operator_mask_generator_size()");

    parser.add_option<shared_ptr<AbstractionMaskGenerator>>(
        "abstraction_mask_generator",
        "determine which abstraction are handled state-dependent",
        "abstraction_mask_generator_size()");

    parser.add_option<int>(
        "max_buckets",
        "Limit the number of bucket to avoid exponential blowup in the number of buckets",
        "infinity",
        Bounds("1", "infinity"));
    ...
    /*
      1. Scale the costs to work with reals up to precision 1 / COST_FACTOR
    */
    ...
    /*
      2. Initialize additional information about task.
    */
    ...
    */
    ...
    shared_ptr<Saturator> diversified_saturator = opts.get<shared_ptr<Saturator>>("diversified_saturator", nullptr);

    /*
      5. Initialize operator mask generator.
    */
    shared_ptr<OperatorMaskGenerator> operator_mask_generator = opts.get<shared_ptr<OperatorMaskGenerator>>("operator_mask_generator");

    /*
      6. Initialize abstraction mask generator.
    */
    shared_ptr<AbstractionMaskGenerator> abstraction_mask_generator = opts.get<shared_ptr<AbstractionMaskGenerator>>("abstraction_mask_generator");

    /*
      7. Initialize the state-dependent data structure.
    */
    CostFunctionStateDependent cost_function_state_dependent(task_info, bdd_builder, opts.get<int>("max_buckets"), opts.get<bool>("diversify"));
