...
CostPartitioningHeuristic compute_saturated_cost_partitioning(
    ...
    vector<int> remaining_ocf = ocf;
    for (int pos : order) {
        const Abstraction &abstraction = *abstractions[pos];
        vector<int> h_values = abstraction.compute_goal_distances_ocf(remaining_ocf);
        vector<int> socf = abstraction.compute_saturated_costs_ocf(h_values);
        cp_heuristic.add_h_values(pos, move(h_values));
        reduce_costs(remaining_ocf, socf);
    }
    cout << "Total time to compute default cp: " << timer() << "\n";
    return cp_heuristic;
}
...
static shared_ptr<Heuristic> _parse(OptionParser &parser) {
    ...
    return get_max_cp_heuristic(parser);
}

static Plugin<Evaluator> _plugin("saturated_transition_cost_partitioning", _parse);
}
