...
void OrderGeneratorGreedy::initialize(
    ...
    for (int abstraction_id = 0; abstraction_id < (int)abstractions.size(); ++abstraction_id) {
        ...
        vector<int> h_values = abstraction.compute_goal_distances_ocf(costs);
        vector<int> saturated_costs = abstraction.compute_saturated_costs_ocf(h_values);
        // move information.
...
static Plugin<OrderGenerator> _plugin_greedy("cp_greedy_orders", _parse_greedy);
}
