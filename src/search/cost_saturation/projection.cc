...
Projection::Projection(
    ...
    match_tree_forward = utils::make_unique_ptr<pdbs::MatchTree>(
...
vector<bool> Projection::compute_reachability_from_state(
    ...
    while (!open.empty()) {
        ...
        for (int op_id : applicable_op_ids) {
            ...
            if (!reachable[successor] &&
                costs[abstract_backward_operators[op_id].concrete_operator_id] != INF) {
...
vector<bool> Projection::compute_reachability_to_state(
    const vector<int> &costs, int state_id) const {
    vector<bool> reachable(get_num_states(), false);
    vector<int> open = {state_id};
    reachable[state_id] = true;
...
vector<int> Projection::compute_goal_distances_for_non_negative_costs(
